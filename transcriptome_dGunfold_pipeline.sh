...
if [ -e $var2 -a -e $var4 ]; then #both files
  python3 transcriptome_dGunfold_1_b_read_gb_file_with_both.py $var1 $var2 $var4 $var5


else #neither file
  python3 transcriptome_dGunfold_1_a_read_gb_file.py $var1 $var5

fi

python3 transcriptome_dGunfold_1_c_generating_seqs_file.py >1_list_of_sequences.txt

python3 transcriptome_dGunfold_1_d_generating_start_sites_file.py >1_list_of_start_positions.txt

RNAfold --temp $var3 <1_list_of_sequences.txt >2_sequences_vienna_output.txt

python3 transcriptome_dGunfold_2_dot_bracket_format.py 1_list_of_sequences.txt >2_dot_bracket_format.txt
...
python3 transcriptome_dGunfold_3_constraints.py 1_list_of_sequences.txt 1_list_of_start_positions.txt >3_sequences_with_constraint.txt

RNAfold --temp $var3 -C <3_sequences_with_constraint.txt >4_sequences_with_constraint_vienna_output.txt

python3 transcriptome_dGunfold_4_calculate_ddG.py $var5 >transcriptome_dGunfold_output1_final.txt

python3 transcriptome_dGunfold_5_summary_table.py >transcriptome_dGunfold_output2_summary_table.txt
